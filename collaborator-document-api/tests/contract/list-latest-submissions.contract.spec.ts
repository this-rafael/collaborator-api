...
import {
  loadListLatestSubmissionsSliceFromContract,
  loadListLatestSubmissionsSliceFromExpected
...
describe("Published latest submissions contract", () => {
  ...
  it("keeps the feature contract synchronized with the normative expected operation", () => {
    const feature = loadListLatestSubmissionsSliceFromContract();
    const expected = loadListLatestSubmissionsSliceFromExpected();

    expect(feature.operation).toEqual(expected.operation);
  });

  it("publishes the GET collection with pagination, cache, and operational failures", async () => {
    const expected = loadListLatestSubmissionsSliceFromContract();
    const operation = await publishedOperation("/api/v1/submissions/latest", "get");

    expect(operation.operationId).toBe(expected.operation.operationId);
    expect(operation.operationId).toBe("listLatestSubmissions");
    ...
    expect(responseSchemaReference(operation, "200", "application/hal+json")).toEqual({
      $ref: "#/components/schemas/LatestSubmissionCollection"
    });
