...
import {loadGetDocumentTypeSliceFromExpected} from "../helpers/openapi-slice.js";
import {
...
describe("Published get document type contract", () => {
  ...
  it("publishes identifier, cache, HAL representation, and failure responses", async () => {
    const operation = await publishedOperation("/api/v1/document-types/{id}", "get");
    expect(operation.operationId).toBe(
      loadGetDocumentTypeSliceFromExpected().operation.operationId
    );
