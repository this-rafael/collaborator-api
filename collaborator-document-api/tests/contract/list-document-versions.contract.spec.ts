...
import {loadListDocumentVersionsSliceFromContract} from "../helpers/openapi-slice.js";
import {
...
describe("Published list document versions contract", () => {
  beforeAll(PlatformTest.bootstrap(Server, contractServerSettings));
  afterAll(PlatformTest.reset);

  it("publishes the GET collection with ordering, pagination, cache, and failures", async () => {
    const expected = loadListDocumentVersionsSliceFromContract();
    ...
    expect(operation.operationId).toBe("listDocumentVersions");
    expect(parameterNames(operation).filter((name) => name !== "id")).toEqual([
      "If-None-Match",
      "cursor",
      "limit",
      "order"
