...
import type {ListPendingDocumentTypeStatisticsInput} from "../../../application/queries/list-pending-document-type-statistics.query.js";
...
export class PendingDocumentTypeStatisticsController {
  ...
  async list(
    ...
    const input: ListPendingDocumentTypeStatisticsInput = {
      ...(cursor !== undefined ? {cursor} : {}),
      limit,
      ...(after ? {after} : {})
    };
    const result = await this.runtime.listPendingDocumentTypeStatistics.execute(input);
    result.match(
...
function invalidCursorOrLimit(field: "cursor" | "limit"): ReportingFailure {
  return {
    ...
    message: "One or more pending document type statistic query parameters are invalid.",
    errors: [
      {
        field,
